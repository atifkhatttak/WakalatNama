...
namespace Business.BusinessLogic
{
    public class SettingsRepository : ISettingsRepository
    {
        ...
        public async Task<List<RejectionReason>> GetRejectionList()
        {
            return await ctx.RejectionReasons.ToListAsync();
        }

        public async Task<bool> UploadBannerContent(UserBannerVM vm)
        {
            if (vm!=null)
            {
                await ctx.UserBanners.AddAsync(new UserBanner()
                {
                    Content = vm?.Content,
                    ExpDate = vm?.ExpDate,
                    RoleId=vm?.RoleId,
        ...
        public async Task<List<UserBannerVM>> GetBannerList(int RoleId)
        {
            var CurrDate = DateTime.Now;
            return await ( (RoleId == 0) ? ctx.UserBanners.Select(x => new UserBannerVM()
            {
                ID = x.ID,
                RoleId = x.RoleId,
                Content = x.Content,
            ...
            :
            ctx.UserBanners.Where(x => x.RoleId == RoleId && x.ExpDate>=CurrDate).Select(x => new UserBannerVM()
            {
                ID = x.ID,
                RoleId = x.RoleId,
                Content = x.Content,
        ...
        {
            return await ctx.UserBanners.Where(x=>x.ID==BannerId).ExecuteDeleteAsync()>0;
        }
